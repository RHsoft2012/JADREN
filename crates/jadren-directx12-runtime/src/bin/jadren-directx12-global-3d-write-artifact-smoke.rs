...
use jadren_directx12_runtime::{Global3dWriteArtifactConfig, run_global_3d_write_artifact_smoke};
use jadren_jir::{
...
fn main() {
    let module = global_3d_write_module();
    match run_global_3d_write_artifact_smoke(
        &module,
        FunctionId::new(0),
        SpirvOptions::new([4, 4, 2]).expect("3D smoke workgroup is valid"),
        Global3dWriteArtifactConfig {
            value: VALUE,
            width: WIDTH,
            height: HEIGHT,
            depth: DEPTH,
        ...
        Ok(report) => println!(
            "{}",
            serde_json::to_string(&report).expect("3D DX12 artifact report is serializable")
        ),
        Err(error) => {
            eprintln!("DX12 global-3d artifact smoke failed: {error}");
            std::process::exit(1);
        }
    }
}

fn global_3d_write_module() -> Module {
