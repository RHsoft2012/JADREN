...
impl ModuleCatalog {
    ...
    /// Returns all module interfaces in deterministic module-name order.
    ///
    /// The compiler uses this view when a source imports a function whose
    /// signature mentions a type declared by another module.  Such a type is
    /// part of the package ABI even when it is not imported by name in the
    /// current source file.
...
impl<'source> Resolver<'source> {
    ...
    fn install_builtins(&mut self, root: ScopeId) {
        ...
        const VALUES: &[&str] = &[
            ...
            "http_request_chunked_frame_length_prefix",
