...
Install the packages from the Unity Package Manager:

1. Open **Window → Package Manager**.
2. Select **+ → Add package from disk...**.
3. Choose `package.json` in `unity/com.jadren.unity`.
4. Repeat for `unity/com.jadren.animation` when animation or crowd rendering
   is required.
